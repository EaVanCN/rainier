...
Rainier.prototype.compiler = function(){         //将页面上指令进行提取,将{{}}转换为指令，并用span包裹。（预处理阶段），获取_data,DOM元素，指令的对应,(v-model在这个地方做对应？)
    ...
    for (var i = 0, length = this._ele.length; i < length; i++){
        ...
        html = html.replace(/\{\{(.*?)\}\}/g, function(a, b){
            if(self._data.hasOwnProperty(b)){
    ...
    spans = document.querySelectorAll("[ra-textNode],[ra-text]");    //ra-text中的值和ra-textNode中需要创建新的文本节点，才能够获得节点、viewmodel的对应关系，所以直接在这里创建，并移除指令。
    for (var j = 0, l = spans.length; j < l; j++) {
        ...
        if(span.getAttribute("ra-type")){                                       //判断是新增的文本节点，还是通过ra-text指令产生的文本节点
            this.reactData[vm_name].push({node:textEle,cmd_key:"ra-textNode",cmd_val:this._data[vm_name]});
            span.parentNode.replaceChild(textEle,span);
        }else{
            this.reactData[vm_name].push({node:span,cmd_key:"ra-text",cmd_val:this._data[vm_name]});
